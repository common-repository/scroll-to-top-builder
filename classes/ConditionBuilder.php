...
class ConditionBuilder {
	...
	public function render() {
		...
		$content = '<div class="ystp-conditions-wrapper" data-child-class="'.$childClassName.'">';
	...
	private function renderConditions() {
		...
		if(empty($savedData)) {
			return '<div>'.__('No Data', YSTP_TEXT_DOMAIN).'</div>';
		}
		$conditions = '';
	...
	private function getConditionRowFromCurrentData($data, $currentIndex) {
		...
		$conditions = '<div class="ystp-condion-wrapper row" data-value-from-first="'.$valueFromFirst.'" data-condition-id="'.esc_attr($currentIndex).'">';
	...
	private function renderConditionRowHeader($name, $currentData, $attributes) {
		$fieldHtml = '<div class="ystp-condition-header">';

		if(!empty($attributes['label'])) {
			$fieldHtml .= '<label>'.__($attributes['label'], YSTP_TEXT_DOMAIN).'</label>';
		}
		$fieldHtml .= '</div>';
	...
	private function renderCondtionConfig($currentConditionIndex, $data) {
		...
		$html = '<div class="col-md-3"><div class="ystp-condition-header ysp-conditions-buttons"><label></label></div>';
		$html .= '<div data-condition-id="'.esc_attr($currentConditionIndex).'" class="ystp-condition-add btn btn-primary">'.__('Add', YSTP_TEXT_DOMAIN).'</div>';
		$html .= '<div data-condition-id="'.esc_attr($currentConditionIndex).'" class="ystp-condition-delete btn btn-danger">'.__('Delete', YSTP_TEXT_DOMAIN).'</div>';
		$html .= '</div>';
